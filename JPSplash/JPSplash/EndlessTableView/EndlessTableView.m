...
#define AUTO_SCROLLING_DEFAULT_VALUE 0.5f
...
-(void)startAutoScrollingWithDelay:(BOOL)isDelay
{
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        if (!self->_timerAutoScrolling)
        {
            __weak id weakSelf = self;
            self->_timerAutoScrolling = [NSTimer autoScrolling_scheduledTimerWithTimeInterval:0.05 block:^{
                EndlessTableView * strongSelf = weakSelf;
                [strongSelf autoScrollingAction];
            } repeats:YES];
            
            [[NSRunLoop mainRunLoop] addTimer:self->_timerAutoScrolling forMode:UITrackingRunLoopMode];
        }
    });
}
...
-(void)autoScrollingAction
{
    if(_upToDown){
        [UIView animateWithDuration:0.05 delay:0.0f options:UIViewAnimationOptionAllowUserInteraction animations:^{
            ...
            [self setContentOffset:CGPointMake(self.contentOffset.x, self.contentOffset.y - (self->_autoScrollValue <= 0.0f ? AUTO_SCROLLING_DEFAULT_VALUE : self->_autoScrollValue))];
...
-(void)setDifferenceRateValue:(CGFloat)differenceRateValue
{
    ...
    if(_attachedTableView) {
        ...
        [_attachedTableView beginUpdates];
        [_attachedTableView reloadData];
        [_attachedTableView endUpdates];
    }
}
